...
Every delivery is an HTTP `POST` to your URL with a JSON body and three `X-LS-Webhook-*` headers:

| Header | Meaning |
| --- | --- |
| `X-LS-Webhook-Event-Type` | The event type, e.g. `connection.active`. Lets you route before parsing. |
| `X-LS-Webhook-Timestamp` | Unix seconds when we sent the delivery. Used in signature checks and replay protection. |
| `X-LS-Webhook-Signature` | HMAC-SHA256 hex of the **raw** request body, keyed with your signing secret. |
...
Never trust a delivery you haven't verified. Compute HMAC-SHA256 over the **raw, unparsed** request body using your signing secret, and compare it to `X-LS-Webhook-Signature` in constant time. Also reject anything whose `X-LS-Webhook-Timestamp` is more than five minutes old — that stops replay attacks.

<Warning>
Verify against the **raw bytes** of the body, exactly as received. If your framework parses JSON and re-serializes it, the bytes change and the signature won't match. Capture the raw body before any JSON middleware touches it.
</Warning>
...
```json connection.capability_changed
{
  ...
  "data": {
    ...
    "observed_at": "2026-07-07T14:22:05Z"
  }
}
```

`previous_status` and `status` tell you the direction of the flip; `consecutive_observations` is how many in a row triggered it (3 for a fail, 2 for a recovery); `observed_at` is when the deciding observation landed.
