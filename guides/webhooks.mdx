...
| `connection.active` | The user finished linking, or a connection recovered from `requires_action` or `failed`. Data is flowing. Carries the **canonical** connection id. |
...
| `connection.disconnected` | The source stopped aggregating the connection (for example MX reports the member disconnected or closed), or you deleted it with `DELETE /v3/connections/{id}`. Re-link with reauthorize, or re-initiate. |
...
<Warning>
There is also **no v3 account-added event**. `connection.active` is not a reliable account-addition signal: it may be absent or emitted again without proving the account set changed. `account.refresh.completed` tells you a refresh finished. Compare paginated account lists before and after the flow to determine which LedgerSync account IDs are new to your integration.

To find new account records, re-list all pages of `GET /v3/accounts?client_id=...` and diff on account ID. Add `connection_id` only when you deliberately want to exclude accounts on other connections, including a new connection created by a changed bank login or source. See [Adding a new account](/guides/adding-accounts).
</Warning>
...
{
  "event_id": "evt_6d84cf35-b2ee-4734-b408-4e6f10fedad8",
...
<CodeGroup>
```bash Read delivery health
curl https://api-sandbox.ledgersyncappv2.com/v3/webhooks/subscriptions/whk_7c1e4a9b83d24f6ab05e2f8c91d3a640 \
  -H "Authorization: Bearer sk_test_..."
```
...
```json Response
{
  "id": "whk_7c1e4a9b83d24f6ab05e2f8c91d3a640",
  "url": "https://yourapp.com/webhooks/ledgersync",
  "event_types": ["connection.active", "connection.failed"],
  "status": "active",
  "created_at": "2026-09-03T14:22:08Z",
...
| Field | Means |
| --- | --- |
| `last_success_at` | The last time we got a `2xx` from your endpoint. |
| `last_failure_at` | The last time a delivery failed: a non-`2xx`, a timeout, or a connection error. |
...
{
  "event_id": "evt_7929029e-bc8f-49c2-a8ee-9c8cdcd3196e",
