...
| `param` / `errors[]` | On `validation_failed`, names the offending field(s). **Absent on most other 400s** — `invalid_request` names the field inside `message` instead, as `Field 'cursor': ...`. Read `message` as the fallback. |
...
| `invalid_request` | 400 | Rejected for a non-schema reason (bad state transition, unsupported source, malformed cursor). The offending field is named inside `message`, as `Field 'cursor': ...`; `error.param` is not set. | Read `message`, fix the named field, resubmit. |
