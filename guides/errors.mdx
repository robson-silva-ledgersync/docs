...
| `409` | Conflict (stale `row_version`) | Re-fetch, then retry with fresh token/body. |
...
| `idempotency_conflict` | 409 | Same `Idempotency-Key`, different body/path. **Reserved: never returned today** (see the note below). | Reuse the exact original body to retry, or a new key for a new request. |
...
<Warning>
  **`Idempotency-Key` is reserved and not honored yet.** The header is accepted
  and ignored: no response is replayed and `idempotency_conflict` is never
  returned. Do **not** auto-retry a failed `POST` on the assumption that the key
  protects you — a retried `POST /v3/clients` or
  `POST /v3/clients/{id}/connections` creates a second one. Send the header if
  you want to be ready for it, but make retries safe on your side, and reconcile
  by listing before you re-create.
</Warning>
