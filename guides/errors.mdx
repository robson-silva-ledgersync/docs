...
| `param` | On every `400` or `413` that is about one request field, the field that tripped the check: `validation_failed` and `invalid_request` alike (dot-separated path for nested fields, e.g. `client.email`). The same name also appears inside `message`, as `Field 'cursor': ...`, so `message` remains a valid fallback. Absent on errors that are not about a field (auth, not found, conflicts, upstream faults). Before 2026-09-11 only schema `validation_failed` carried it; `invalid_request` named the field inside `message` only. |
...
| `invalid_request` | 400 | Rejected for a non-schema reason (bad state transition, unsupported source, malformed cursor, an id with the wrong prefix). `error.param` names the offending field, and `message` repeats it as `Field 'cursor': ...`. | Fix the field named in `param`, resubmit. |
