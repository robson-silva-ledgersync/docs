...
| --- | --- | --- |
| `widget_url` | `widget_url`, `expires_at` | **The only kind emitted today.** Open the URL in a browser, iframe, or webview. |
| `reauthorize` | `reauth_url`, `expires_at` | Reserved for a future release — not emitted today. |
...
<Note>
Because `mfa_challenge` and `reauthorize` are not emitted yet, a live connection that needs the user — whether for first-time MFA, a step-up challenge, or reauthorization after a password change — always surfaces `action.kind = "widget_url"`. Branch on `kind` anyway so your integration keeps working when the other kinds ship.
</Note>
