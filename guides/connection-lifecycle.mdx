...
stateDiagram-v2
    ...
    requires_action --> failed: bank or aggregator failure
    active --> requires_action: credentials, MFA or reauthorization needed
    active --> disconnected: link removed at the source, or deleted
    active --> failed: bank or aggregator failure
    failed --> active: recovers on a later refresh
...
| `disconnected` | The connection no longer refreshes: the source stopped aggregating it (for example MX reports the member disconnected or closed), you deleted it with `DELETE /v3/connections/{id}`, or, for Finicity and portal connections, LedgerSync's own refresh of it failed. | Show a re-link CTA. **Reauthorize in place** to keep your ids, or re-initiate. |
...
<Note>
  **GET and the webhooks report the same status.** An MX connection's `status` comes from MX's own member
  status, the same one the `connection.*` webhooks report, and `last_error` says why. For example, MX
  `PREVENTED` (too many failed sign-ins) and `DENIED` read `requires_action` with `last_error.code`
  `credentials_invalid`; MX `DISCONNECTED` and `CLOSED` read `disconnected`; a healthy or still-syncing
  member reads `active`. Before 2026-09-23 the read endpoint reported any unhealthy MX connection as
  `disconnected`. Expect those MX connections to read their real status now.

  One MX pairing is deliberate: an MX account the bank **locked** reads `requires_action` with `last_error.code`
  `account_locked_by_institution` and `is_user_actionable: false`. The user has to act, but at the bank,
  not in the widget. (A locked Finicity account on an existing connection reads `failed` with the same code; the sandbox `locked` login does not simulate one.)
</Note>
...
| `reauthorize` | `reauth_url`, `expires_at` | Returned on demand by [`POST /connections/{id}/reauthorize`](#handling-failed-and-disconnected) to repair an existing connection. Not gated on status, but Finicity Connect Fix may report "No action required" on a healthy connection. Use [Full Connect to add Finicity accounts](/guides/adding-accounts#finicity-open-full-connect-for-the-existing-client). Not emitted on a `requires_action` connection's own `action`. |
...
### Reauthorize an existing connection (recommended for repair)
...
The endpoint does **not** reject a request just because the connection is `active`. That is separate from whether the provider has a repair for the user to complete. On Finicity, the URL uses **Connect Fix** and can show **"No action required"** on a healthy connection.

**Use Full Connect to add Finicity accounts.** Call `POST /v3/clients/{client_id}/connections` for the existing client, obtain the operation's `result.connection.action.widget_url`, and choose the existing bank's add-account option. The same institution login reuses the existing connection. A v3 Finicity refresh only updates stored accounts; MX and FDE refresh paths also import newly returned accounts. See [Adding a new account](/guides/adding-accounts) for the complete flow and account-list comparison.
...
Coverage differs by source, and within a source by bank. Measured in September 2026 on credit cards whose connection was working, as the share of cards where each field is present:

| Source | What it reports |
| --- | --- |
| Finicity | All five fields, where the bank publishes them. Minimum payment 91%, due date 91%, statement balance 90%, available credit 77%, credit limit 76%. |
| MX | All five fields, where the bank publishes them. Minimum payment 66%, due date 64%, available credit 64%, credit limit 61%. The statement balance is reported since 2026-09-23 and is not measured yet. |
| Portal connections (FDE) | None, so `liabilities` is always null |
| Uploaded statements | None, so `liabilities` is always null |

The bank matters more than the source. Through MX, Chase, Bank of America, Wells Fargo, U.S. Bank and Citi report the terms on nearly every card, while American Express reports the statement balance and, on very few cards, anything else. Through Finicity, Capital One reports no credit limit and no available credit.

Terms are refreshed whenever the account's balance is, including on LedgerSync's scheduled daily refresh.
