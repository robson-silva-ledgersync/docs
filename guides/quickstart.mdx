...
<Step title="Register a webhook subscription">
  ...
  The easiest path is the **portal Webhooks page** — add your endpoint, pick events, and see deliveries and retries visually. To do it over the API, subscribe to all seven event types:
...
<Step title="Create a Client">
  ...
  Hold onto `cli_01HXYZ7ABCDEF...` — every read later is scoped to it. `phone` is optional; add `"sms_invitation": true` alongside a non-blank `phone` to have LedgerSync text the client a login invitation on creation.
</Step>
...
`amount` is a **signed passthrough** of the source aggregator's value — LedgerSync does not flip or normalize it. In practice the convention you'll see is:
...
<Warning>
  Because the sign is a passthrough, treat it as source-driven rather than a LedgerSync guarantee. Before you post amounts into a ledger, confirm the sign against your own sandbox data with FinBank — the fixtures above are exactly what the sandbox returns, so a quick read tells you which way is which for your integration.
</Warning>
