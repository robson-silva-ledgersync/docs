...
<Step title="Register a webhook subscription">
  ...
  The easiest path is the **portal Webhooks page**: add your endpoint, pick events, and see deliveries and retries visually. Each subscription also reports its own delivery health over the API, so you can monitor it from your code too. See [Check delivery health](/guides/webhooks#check-delivery-health). To do it over the API, subscribe to all eight event types:
  ...
  That returns `{ "event_id": "evt_6d84cf35-b2ee-4734-b408-4e6f10fedad8" }` and nothing to poll — watch your endpoint for the `webhook.test` delivery. The id you get back is exactly what the delivery carries, both in its `X-LS-Webhook-Event-Id` header and in the envelope's `event_id`, so you can match them up as-is.
</Step>
