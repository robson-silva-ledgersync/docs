...
There are two ways to connect a user's bank for the first time. Both finish with the same
`connection.active` webhook and the same data behind it. Choose based on how
much UI you want to own.

<Note>
Adding an account to an existing Finicity bank uses **Full Connect** for the same client. Call `POST /v3/clients/{client_id}/connections`, open the operation's widget URL, and use the existing bank's add-account option. A matching institution login reuses the stored connection. Follow [Adding a new account](/guides/adding-accounts) for this flow; it uses account-list comparison to detect additions and does not rely on another `connection.active` event.
</Note>
...
<Steps>
  <Step title="Create the member as a Client">
    ...
    curl https://api-sandbox.ledgersyncappv2.com/v3/clients \
      ...
      -d '{"email":"alice@example.com","name":"Alice","phone":"5551234567"}'
    ```

    <Note>
      Add `"phone"` to store a contact number. To have LedgerSync **text**
...
<Warning>
  **A link you already emailed cannot be revoked through the API today.** The
  reference documents `DELETE /clients/{id}/connect-session/{sid}`, but the
  create call returns only `url` and `expires_at`, and there is no endpoint that
  lists sessions, so there is no supported way to obtain the `sid` it needs.
  Treat `expires_at` as the only expiry, and keep the window short if that
  matters to you.
</Warning>
...
The wrapper needs **no Permissions-Policy `allow` attributes** — it uses no
camera, microphone, or geolocation. A plain iframe works; `widget_url` is the URL
`POST /clients/{id}/connections` hands back:

```html
<iframe src="{widget_url}" title="Connect your bank"></iframe>
```

It's a first-party, frame-guarded page, so you don't need a `sandbox` attribute.
...
<Warning>
  **OAuth banks can't complete inside a frame.** Institutions that use OAuth
  (many large banks) block being iframed, so the wrapper opens the bank's OAuth
  page in a **new tab** (`window.open`) and the member finishes there. So allow
  popups if you sandbox, and don't assume the whole flow stays in your frame.
  Credential-based banks complete fully in-frame. Either way you get the same
  `ls/mx/memberConnected` `postMessage` (and the `ls_token` redirect, if you set
  one) once the connection succeeds.
</Warning>

Detecting completion **without a redirect** is exactly what the `postMessage` is
