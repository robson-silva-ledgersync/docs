...
Read it per account rather than assuming a fixed window, because depth varies a lot:

- **A newly linked bank connection starts with roughly the last quarter.** History then grows *forward* from there with each refresh, so an account linked a year ago holds about a year.
- **Uploaded statements go back as far as the statements do.** A client who uploaded three years of PDFs has three years, available immediately.
- **It is `null` when we hold no transactions for that account yet**, which is the normal state between linking a connection and its first transaction pull finishing.
