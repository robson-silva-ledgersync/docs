...
Each Account exposes a `current_balance`, plus its transactions through `GET /accounts/{id}/transactions`. This guide explains how fresh that data is, how to tell, and how to pull fresher data on demand. For what `current_balance` means and how it is signed, see [Asset vs liability](/guides/connection-lifecycle#asset-vs-liability).
...
Freshness is one axis; depth is the other. Every Account carries an **`oldest_transaction_date`**: the oldest transaction we actually hold for it.
...
| Source | History requested on first connect |
| --- | --- |
| Finicity | The last **90 days** |
| MX | The last **730 days**. MX returns what it holds, which is often less |
| Portal connections (FDE) | Not windowed. You get what the extraction run returns |
| Uploaded statements | Exactly the statements that were uploaded |
...
What actually arrives inside the requested window is up to the institution, and it is not something we can tell you before the connection exists. That is what the field is for: read `oldest_transaction_date` per account instead of assuming a window. It is `null` when we hold no transactions for that account yet, which is normal between linking a connection and its first transaction pull finishing.

<Note>
  `oldest_transaction_date` can move **backwards**. It is a fact about what we hold right now, and a backfill landing earlier transactions lowers it. Re-read it rather than caching it as a fixed property of the account.
</Note>

Like `Transaction.date`, it is a calendar day (`YYYY-MM-DD`) with no time component, and it is truncated by the same code, so comparing the two is safe.
