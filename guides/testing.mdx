...
| Invalid credentials | `customer1` | `bad` | n/a | `connection.requires_action`, `last_error.code = credentials_invalid`. |
...
| `connection.failed` | The bank or the aggregator failed in a way the user cannot fix: institution-side outage, the bank refusing automated access, or an upstream pipeline error. A Finicity-locked account is `failed` too. Rejected credentials (Finicity `customer1` / `bad`, MX `mxuser` / `UNAUTHORIZED`) and an MX-locked account are `requires_action`, not `failed`. | Finicity `locked` / `go`, or MX `mxuser` / `UNAVAILABLE` or `SERVER_ERROR`. `last_error` carries a stable code. |
...
- **Invalid credentials → `connection.requires_action`** — initiate FinBank, sign in `customer1` / `bad`. Watch `last_error.code = credentials_invalid` land. (Or link **FinBank Decertified** for a decertification `last_error`.)
- **MFA wrong answer → `connection.failed`** — initiate Finicity, sign in `customer3` / `go`, submit any OTP other than `999999`.
- **MX locked user** — initiate MX, sign in `mxuser` / `LOCKED`. `connection_status = LOCKED`; the connection reads `requires_action` with `last_error.code = account_locked_by_institution` and `is_user_actionable: false`.
- **Upstream 503 → `connection.failed`** — initiate MX, sign in `mxuser` / `UNAVAILABLE`. Simulates an institution-side 503; `last_error.code = institution_unavailable`, retryable.
